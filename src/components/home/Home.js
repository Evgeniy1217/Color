...
const Home = () => {
    ...
    const colorsL = gradientL.map((el, ind) =>
        <div key={keysL++}
             style={{
            ...
            {el.gradientsL}
            <svg onClick={() => {
                dispatch({type: "DISTRIBUTE_HEX_L", payload: colorsL[ind].props.style.backgroundColor});
                navigate('/edit')
            }}
    ...
    const colorsR = gradientR.map((el, ind) =>
        <div key={keysR++}
             style={{
            ...
            <svg onClick={() => {
                dispatch({type: "DISTRIBUTE_HEX_R", payload: colorsR[ind].props.style.backgroundColor});
                navigate('/edit')
            }}
                 xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                 className="rounded float-end bi bi-pencil-fill" viewBox="0 0 16 16">
